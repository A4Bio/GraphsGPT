...
class GraphsGPTEncoderConditioned(nn.Module):
    ...
    def concat_properties(
        ...
        if values_lipinski is not None:
            values_properties.append(values_lipinski)
            indices_properties.append(2)
        if values_logp is not None:
            values_properties.append(values_logp)
            indices_properties.append(3)
    ...
    def forward(
        ...
        values_properties, indices_properties = self.concat_properties(
            ...
            values_sa=values_sa,
            values_lipinski=values_lipinski,
        ...
        property_position_embeds = self.embed_property_position(
            torch.arange(0, num_properties, device=device).expand(batch_size, num_properties)
        )  # (batch_size, num_properties, embed_dim)
