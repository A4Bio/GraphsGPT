...
from utils.property_scores.nspdk import get_npsdk
...
def main(args):
    ...
    """load data & model"""
    train_file = "data/zinc250k/all_250k_rndm_zinc_drugs_clean_3.txt"
    valid_file = "data/zinc250k/valid_250k_rndm_zinc_drugs_clean_3.txt"

    with open(train_file, "r") as file:
        train_smiles = [data.replace("\n", "") for data in file.readlines()]
        random.shuffle(train_smiles)
    with open(valid_file, "r") as file:
        valid_smiles = [data.replace("\n", "") for data in file.readlines()]
        ...
        while len(finished_shots) < args.num_shots:
            ...
            for shot_id, candidate_shot in tqdm(candidate_shots.items(), desc="Aggregating results"):
                if candidate_shot["sample_times"] >= args.max_sample_times or len(candidate_shots[shot_id]["generation_results"]) >= args.num_samples_each_shot:  # exceed max trys / results are enough
                    ...
                    this_iter_finished_results.extend(candidate_shots[shot_id]["generation_results"][:args.num_samples_each_shot])  # add a maximum of "num_samples_each_shot" results
    ...
    metrics = moses.get_all_metrics(
        ...
        test=valid_smiles,
        test_scaffolds=valid_smiles,
    )

    # 🔍 NPSDK
    npsdk_mmd = get_npsdk(all_smiles, valid_smiles, n_jobs=args.num_processes)
    metrics["NPSDK"] = npsdk_mmd
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", default="./results/unconditional/zinc250k", type=str, help="Path to save the evaluation results.")
